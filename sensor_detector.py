...
from datetime import datetime, timezone
from typing import Dict, List, Optional, Tuple
from geopy.geocoders import Nominatim  # 👈 Requerido: pip install geopy
...
# Inicialización del geocodificador
geolocator = Nominatim(user_agent="epicentro_seismic_detector")
...
class SeismicCluster:
    ...
    avg_intensity: float
    timestamp_ms: int
    place: str = ""  # 👈 Nuevo campo: Guarda la ciudad/país para tu cliente Android
...
class SeismicSensorDetector:
    ...
    def _resolve_location_name(self, lat: float, lng: float) -> str:
        """Convierte coordenadas geográficas en un nombre de ubicación reconocible para Android."""
        try:
            location = geolocator.reverse(
                (lat, lng), exactly_one=True, language="es", timeout=3)
            if location and location.raw.get("address"):
                address = location.raw["address"]
                city = address.get("city") or address.get("town") or address.get(
                    "state") or address.get("county") or ""
            ...
            logging.error(
                f"⚠️ Error resolviendo ubicación ({lat}, {lng}): {e}")

        return f"Cerca de {round(lat, 2)}, {round(lng, 2)}"
    ...
    def _evaluate_clusters(
        ...
        if report_count >= self.min_reports_for_alert:
            ...
            # 🟢 Obtenemos el nombre legible de la ubicación para la app Android
            place_name = self._resolve_location_name(avg_lat, avg_lng)

            logging.critical(
                f"🚨 ¡ALERTA CONFIRMADA POR RED COMUNITARIA! "
                f"Cluster: {cluster_id} | Lugar: {place_name} | Dispositivos: {report_count} | Intensidad: {
                    avg_intensity:.2f} m/s²"
            )
            ...
            return SeismicCluster(
                ...
                avg_intensity=avg_intensity,
                timestamp_ms=int(time.time() * 1000),
                place=place_name  # 👈 Se envía aquí la cadena resuelta
            )
